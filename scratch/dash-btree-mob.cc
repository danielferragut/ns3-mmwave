...
main (int argc, char *argv[])
{
  ...
  for (unsigned int i = 0; i < network.getLinks ().size (); i += 1)
    {
      ...
      for (size_t i_ipv4 = 1; i_ipv4 < dstipv4->GetNInterfaces (); i_ipv4++)
        {
          same_bcst = srcipv4->GetInterfaceForPrefix (dstipv4->GetAddress (i_ipv4, 0).GetLocal (),
                                                      dstipv4->GetAddress (i_ipv4, 0).GetMask ());
          if (same_bcst != -1)
            {
              break;
            }
  ...
  mobility.SetPositionAllocator ("ns3::GridPositionAllocator", "MinX", DoubleValue (0), "MinY",
                                 DoubleValue (0), "DeltaX", DoubleValue (100.0), "DeltaY",
                                 DoubleValue (100.0), "GridWidth", UintegerValue (400),
  ...
  string representationStrings =
      GetCurrentWorkingDir () + "/../content/representations/netflix_vid1.csv";
  ...
  for (size_t i = 0; i < nodes.GetN (); i++)
    {
      ...
      string strIpv4Edge =
          Ipv4AddressToString (edgeServer->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ());

      DASHServerHelper edgeServerCache (Ipv4Address::GetAny (), 80, strIpv4Edge, "/content/mpds/",
                                        representationStrings, "/content/segments/");
  ...
  for (auto &client : m_clients)
    {
      ...
      string strIpv4Lcl =
          Ipv4AddressToString (clientNode->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ());
      string strIpv4Bst =
          Ipv4AddressToString (clientNode->GetObject<Ipv4> ()->GetAddress (1, 0).GetBroadcast ());
      ...
      Simulator::Schedule (Seconds (start), &DashController::AddUserInGroup, controller, apId,
                           dst_server, 1, userId);
    }
  fileMobility.flush ();
  fileMobility.close ();
